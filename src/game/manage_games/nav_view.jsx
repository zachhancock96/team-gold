...
/*
  ...
  All Games/Approved Games/Pending Games/Rejected Games

  @see src/navbar/controller.jsx, src/navbar/view.jsx and @see src/navbar/game/manage_games/nav_controller.jsx
...
export const NavView = ({links, activeLinkId, onLinkClick}) => {
  ...
  const linkItems = links.map(link => {
    return <span key={link.id}><a 
      href="#"
      onClick={e => { e.preventDefault(); onLinkClick(link.id); }}>{link.name}</a> / </span> 
  });

  return <div className='nav2'>{linkItems}</div>
}