(function () {
	 ...
	 */
	var commonSetup = ({ stream, accumulator, combine, app }) => {
	  if (!stream) {
	...
	function createRouteMatcher (routes, fallback) {
	...
	var routerHelper = createCommonjsModule(function (module, exports) {
	/**
	...
	unwrapExports(routerHelper);
	var routerHelper_1 = routerHelper.findPathParams;
	var routerHelper_2 = routerHelper.findQueryParams;
	var routerHelper_3 = routerHelper.setParams;
	var routerHelper_4 = routerHelper.convertToPath;
	var routerHelper_5 = routerHelper.createRouteMap;
	var routerHelper_6 = routerHelper.createRouter;
	var routerHelper_7 = routerHelper.createFeatherRouter;
	var routerHelper_8 = routerHelper.createUrlMapperRouter;
	var routerHelper_9 = routerHelper.createMithrilRouter;
	...
	unwrapExports(state);
	var state_1 = state.createRouteSegments;
	var state_2 = state.findRouteSegmentWithParams;
	var state_3 = state.findRouteSegment;
	var state_4 = state.diffRoute;
	var state_5 = state.routeTransition;
	var state_6 = state.whenPresent;
	var state_7 = state.Routing;
	var state_8 = state.navigateTo;
	var state_9 = state.Actions;

	const service = ({
	  previousState,
	  state
	}) => ({
	  routeTransition: () => state_5(previousState.route, state.route)
	});
	const effect = ({
	  state,
	  update
	...
	const Route = state_1(["Login", "School", "Game"]);
	const routeConfig = {
	  Login: "/login",
	  School: "/school/:id",
	  Game: "/game"
	...
	const redirect = route => {
	  return {
	    redirect: Array.isArray(route) ? route : [route]
	  };
	};

	const router$1 = routerHelper_7({
	  createRouteMatcher,
	  routeConfig,
	...
	const Initial = async () => {
	  const user = await getMe().catch(err => null);
	  return {
	    route: router$1.initialRoute,
	...
	const Login = ({
	  ...
	  return [mithril("div", null, mithril("form", {
	    onsubmit: e => {
	      e.preventDefault();
	      actions.login({
	        name: form.name,
	  ...
	  }, mithril("label", null, "Name:", mithril("input", {
	    typel: "name",
	    value: form.name,
	    oninput: e => actions.updateLoginForm('name', e.target.value)
	  })), mithril("label", null, "Email:", mithril("input", {
	    type: "email",
	    value: form.email,
	...
	const SchoolList = ({
	  ...
	  return [mithril("input", {
	    ...
	    oninput: e => {
	      actions.setSchoolFilter(e.target.value);
	    }
	  }), mithril("ul", null, schools.map(s => mithril("li", {
	    onclick: e => {
	      actions.selectSchool(s.id);
	    },
	...
	const SchoolDetail = (() => {
	  ...
	  return ({
	    ...
	    style: style
	  }, "SchoolName"), mithril("div", {
	    style: style
	  }, "Assignor"), mithril("div", {
	    style: style
	  }, "School Admin"), mithril("div", {
	    style: style
	...
	const getSchools = (() => {
	  const schools = [{
	    ...
	    id: 3,
	    name: 'Mason Dixon High School'
	  }, {
	    id: 4,
	    name: 'Monroe High School'
	  }];
	  return () => {
	    return timeout$1(500).then(() => {
	      return schools;
	    });
	  };
	})();

	function timeout$1(n) {
	  return new Promise(resolve => {
	    setTimeout(resolve, n);
	  });
	}
	...
	const Navbar = ({
	  ...
	  const routing = state_7(state.route);

	  const className = id => routing.localSegment.id === id ? 'navbar-links active' : 'navbar-links';

	  return mithril("div", null, mithril("nav", null, mithril("a", {
	    href: router$1.toPath(Route.School()),
	    class: className('School')
	  }, "School"), mithril("a", {
	    href: router$1.toPath(Route.Game()),
	...
	const Root = ({
	  ...
	  const routing = state_7(state.route);
	  const Component = componentMap[routing.localSegment.id];
	  const navbar = Component.showNavbar ? Navbar({
	    state,
	    actions
	  }) : null;
	  ...
	  return [loading, mithril("div", {
	    style: {
	      display: 'flex',
	      flexDirection: 'column',
	      height: '100vh'
	    }
	  }, state.device === 'desktop' ? [mithril("div", null, navbar), mithril("div", {
	    style: {
	      flex: 1
	    }
	  }, component)] : [mithril("div", {
	    style: {
	      flex: 1
	    }
	  }, component), mithril("div", null, navbar)])];
	};
	...
	const createApp = async () => {
	  ...
	  return {
	    initial,
	    Actions: update => Object.assign({}, routes.Actions(update), login$1.Actions(update), school.Actions(update)),
	    services: [routes.service, login$1.service, school.service],
	...
	*/
	...
	createApp().then(app => {
	  ...
	  states.map(state => {
	    const View =  ViewWithTracer_ ;
	    mithril.render(root, View({
	      app,
	      state,
	      actions
	    })); //`state.route` to browser address bar

	    router$1.locationBarSync(state.route);
	  ...
	  viewport.watchDeviceChange({
	    update,
	    states
	  });
	});

	const ViewWithTracer = states => ({
	  app,
	  state,
	  actions
	}) => {
	  return [mithril("div", {
	    ...
	    state,
	    actions
	  })), mithril("div", null, mithril(TracerComponent, {
	    states: states
	  })))];
	};
